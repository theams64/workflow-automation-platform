...
namespace Backend.Api.Tests.Integration
{
    public sealed class RateLimitedWebApplicationFactory : WebApplicationFactory<Program>
    {
        protected override void ConfigureWebHost(IWebHostBuilder builder)
        {
            ...
            {
                configBuilder.AddJsonFile(Path.Combine(AppContext.BaseDirectory, "appsettings.Test.json"), optional: false, reloadOnChange: false);
            });
            ...
            builder.ConfigureTestServices(services =>
            {
                ...
                services.AddRateLimiter(options =>
                {
                    ...
                    options.GlobalLimiter = PartitionedRateLimiter.Create<HttpContext, string>(httpContext => RateLimitPartition.GetNoLimiter("rate-limit-tests-global"));

                    options.AddPolicy(RateLimitingPolicies.Login, httpContext =>
                        RateLimitPartition.GetFixedWindowLimiter("login-test-partition", _ => 
                    ...
                    options.AddPolicy(RateLimitingPolicies.Registration, httpContext =>
                        RateLimitPartition.GetFixedWindowLimiter("registration-test-partition", _ => 
                            new FixedWindowRateLimiterOptions
                            {
                                PermitLimit = 2,
                                Window = TimeSpan.FromMinutes(1),
                    ...
                    options.AddPolicy(RateLimitingPolicies.TokenRefresh, httpContext =>
                        RateLimitPartition.GetFixedWindowLimiter("refresh-test-partition", _ => 
                            new FixedWindowRateLimiterOptions
                            {
                                PermitLimit = 2,
                                Window = TimeSpan.FromMinutes(1),
                    ...
                    options.AddPolicy(RateLimitingPolicies.PasswordReset, httpContext => RateLimitPartition.GetNoLimiter("password-reset-test-partition"));
                });
            });
        }
