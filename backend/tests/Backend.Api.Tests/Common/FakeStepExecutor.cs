...
namespace Backend.Api.Tests.Common
{
    public sealed class FakeStepExecutor : IWorkflowStepExecutor
    {
        ...
        private readonly Func<string, StepOutputSchema> _getOutputSchema;

        public FakeStepExecutor(string stepType, StepOutputSchema? outputSchema = null, Func<StepExecutionContext, CancellationToken, Task<StepExecutionResult>>? execute = null, Func<string, WorkflowValidationContext, IReadOnlyList<ServiceError>>? validate = null, Func<string, StepOutputSchema>? getOutputSchema = null)
        {
            StepType = stepType;
            ...
            _getOutputSchema = getOutputSchema ?? (_ => OutputSchema);
        }
        ...
        public StepOutputSchema GetOutputSchema(string configJson) => _getOutputSchema(configJson);
